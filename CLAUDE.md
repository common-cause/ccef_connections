...
Reusable Python library providing unified connection management for CCEF data integrations. Connectors: Airtable, OpenAI/ChatGPT, Google Sheets, BigQuery, Snowflake, HelpScout, Zendesk, Zoom, Action Network, Action Builder, Asana, PTV, ROI CRM, Stripe, Geocodio, Google Address Validation, GitHub, Hex, Civis Platform, Email (Resend), Tatango (SMS), User Profile (Entra staff lookup via Power Automate). Uses Civis-compatible {CREDENTIAL_NAME}_PASSWORD env var pattern.
